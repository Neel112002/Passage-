...
class _NotificationSettingsScreenState extends State<NotificationSettingsScreen> {
  ...
  Future<void> _pickTime({required bool isStart}) async {
    final contextPrefs = _prefs;
    final minutes = isStart ? contextPrefs.dndStartMinutes : contextPrefs.dndEndMinutes;
    final initial = TimeOfDay(hour: (minutes ~/ 60) % 24, minute: minutes % 60);
    final picked = await showTimePicker(context: context, initialTime: initial);
    if (picked == null) return;
    final mins = picked.hour * 60 + picked.minute;
    if (isStart) {
      await _save(_prefs.copyWith(dndStartMinutes: mins));
    } else {
      await _save(_prefs.copyWith(dndEndMinutes: mins));
    }
  }
