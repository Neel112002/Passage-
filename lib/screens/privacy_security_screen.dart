...
class _PrivacySecurityScreenState extends State<PrivacySecurityScreen> {
  ...
  Future<void> _toggle2FA(bool v) async {
    await LocalAuthStore.setTwoFactorEnabled(v);
  ...
  Future<void> _toggleAppLock(bool v) async {
    final s = await LocalAuthStore.load();
    if (v && (s.appLockPinHash == null)) {
      final pin = await _promptSetPin();
      if (pin == null) return; // cancelled
      await LocalAuthStore.setAppLock(enabled: true, pin: pin);
    } else {
      await LocalAuthStore.setAppLock(enabled: v);
    }
    final updated = await LocalAuthStore.load();
    if (!mounted) return;
        ...
        return AlertDialog(
          ...
          content: Form(
            ...
            child: TextFormField(
              ...
              validator: (v) {
                final t = (v ?? '').trim();
                if (t.length < 4 || t.length > 6) return 'PIN must be 4-6 digits';
                if (int.tryParse(t) == null) return 'Only digits allowed';
                return null;
              },
            ),
          ),
  ...
  Future<void> _changeEmail() async {
          ...
          actions: [
            ...
            FilledButton(
              onPressed: () async {
                ...
                if (hasPw) {
                  final ok = await LocalAuthStore.verifyPassword(pwController.text);
                  if (!ok) {
                    if (context.mounted) {
    ...
    if (ok == true) {
      ...
      final p = _profile;
      if (p != null) {
        final updated = p.copyWith(email: newEmail);
        try {
          await LocalUserProfileStore.save(updated);
        } catch (_) {}
        try {
          await LocalAuthStore.setLoginEmail(newEmail);
        } catch (_) {}
        if (!mounted) return;
