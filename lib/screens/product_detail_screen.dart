...
class _ProductDetailScreenState extends State<ProductDetailScreen>
  ...
  Future<void> _runAddToCartAnimation(String imageUrl) async {
    try {
      final fromBox =
          _imageGlobalKey.currentContext?.findRenderObject() as RenderBox?;
      final toBox =
          cartIconGlobalKey.currentContext?.findRenderObject() as RenderBox?;
      final overlay = Overlay.of(context);
      if (fromBox == null || toBox == null || overlay == null) return;

      final fromCenter = fromBox.localToGlobal(
          Offset(fromBox.size.width / 2, fromBox.size.height / 2));
      final toCenter = toBox
          .localToGlobal(Offset(toBox.size.width / 2, toBox.size.height / 2));

      final entrySize = 72.0;
      final beginOffset =
          Offset(fromCenter.dx - entrySize / 2, fromCenter.dy - entrySize / 2);
      final endOffset =
          Offset(toCenter.dx - entrySize / 2, toCenter.dy - entrySize / 2);
      ...
      final entry = OverlayEntry(builder: (context) {
        return AnimatedBuilder(
          ...
          builder: (context, _) {
            ...
            final dx = beginOffset.dx + (endOffset.dx - beginOffset.dx) * t;
            // Slight upward arc
            final midY = math.min(beginOffset.dy, endOffset.dy) - 80.0;
            final quadY = (1 - t) * (1 - t) * beginOffset.dy +
                2 * (1 - t) * t * midY +
                t * t * endOffset.dy;
            final size = entrySize * (1 - 0.6 * t);
                ...
                child: SizedBox(
                  ...
                  child: ClipRRect(
                    ...
                    child: Builder(builder: (context) {
                      ...
                      final cw = (entrySize * dpr).round();
                      final ch = (entrySize * dpr).round();
                       if (imageUrl.trim().startsWith('data:image')) {
                        try {
                           final base64Part = imageUrl.trim().split(',').last;
