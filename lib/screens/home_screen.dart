...
class _HomeScreenState extends State<HomeScreen> {
  ...
  // In-memory saved reels
  final Set<String> _savedReels = <String>{};

  // Seller-uploaded local reels (persisted locally)
  List<ReelItem> _localReels = <ReelItem>[];

  // If set, Reels page will open at this index next time it's shown
  int? _reelStartIndex;
  ...
  // Seller profile status for Settings
  bool _sellerExistsForProfileEmail = false;
  bool _checkingSellerForProfile = false;
  final TextEditingController _sellerEmailCtrl = TextEditingController();
  final TextEditingController _sellerPasswordCtrl = TextEditingController();
  final TextEditingController _sellerConfirmCtrl = TextEditingController();
  final TextEditingController _sellerStoreNameCtrl = TextEditingController();
  ...
  void initState() {
    ...
    // Load local reels (seller videos)
    _loadLocalReels();
  ...
  void dispose() {
    ...
    _sellerPasswordCtrl.dispose();
    _sellerConfirmCtrl.dispose();
    _sellerStoreNameCtrl.dispose();
    _scrollIdleDebounce?.cancel();
    _productsSub?.cancel();
    super.dispose();
  }
  ...
  Future<void> _loadProfile() async {
    try {
      ...
      if (p != null && mounted) {
        setState(() => _profile = p);
        // Seed seller email with current profile email and refresh status
        _sellerEmailCtrl.text = p.email;
        await _refreshSellerStatus();
      }
    } catch (_) {
  ...
  Future<void> _refreshSellerStatus() async {
    if (!mounted) return;
    setState(() => _checkingSellerForProfile = true);
    try {
      final exists = await LocalSellerAccountsStore.emailExists(_profile.email);
      if (!mounted) return;
      setState(() {
        _sellerExistsForProfileEmail = exists;
        _checkingSellerForProfile = false;
      });
    } catch (_) {
      if (!mounted) return;
      setState(() {
        _sellerExistsForProfileEmail = false;
        _checkingSellerForProfile = false;
      });
    }
  }

  Future<void> _openCreateSellerSheet() async {
    // Reimplemented as a centered dialog instead of a bottom sheet
    final theme = Theme.of(context);
    _sellerEmailCtrl.text = _profile.email;
        ...
        return StatefulBuilder(
          builder: (ctx, setDialogState) {
            Future<void> submit() async {
              ...
              try {
                final email = _sellerEmailCtrl.text.trim();
                final password = _sellerPasswordCtrl.text;

                // Verify this is the same password as their account by reauthenticating.
                await FirebaseAuthService.reauthenticate(
                    email: email, password: password);

                // Ensure a local seller record exists for compatibility (hash stored locally only).
                final exists =
                    await LocalSellerAccountsStore.emailExists(email);
                if (!exists) {
                  await LocalSellerAccountsStore.addSeller(
                    email: email,
                    name: _profile.fullName.trim().isEmpty
                        ? 'Seller'
                ...
                // Mark session and navigate to Seller Dashboard
                ...
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(content: Text('Welcome to Seller Dashboard')),
                );
              } catch (e) {
                if (!mounted) return;
    ...
    _sellerPasswordCtrl.clear();
    _sellerConfirmCtrl.clear();
    _sellerStoreNameCtrl.clear();
  }

  Future<void> _loadLocalReels() async {
    try {
      final items = await LocalReelsStore.loadAll();
      if (mounted) setState(() => _localReels = items);
    } catch (_) {
      // ignore
    }
  }

  List<_DemoReel> get _combinedReels {
    // Map local reels to _DemoReel by using data URIs.
    const fallbackCover =
        'https://images.unsplash.com/photo-1530915365347-3f8b6d01b401?w=800&h=800&fit=crop';
    final local = _localReels.map((r) {
      final cover = r.coverImageBase64.isNotEmpty
          ? 'data:image/jpeg;base64,${r.coverImageBase64}'
          : fallbackCover;
      final video = 'data:video/mp4;base64,${r.videoBase64}';
      return _DemoReel(
          id: 'lr_${r.id}',
  ...
  // Remove demo reels; keep empty until seller uploads local reels
  final List<_DemoReel> _reels = const [];
  ...
  void _toggleSaveReel(String reelId) {
    setState(() {
  ...
  void _maybePrecacheTopGridImages(
      BuildContext context, List<_DemoProduct> products) {
    // Use the same key as filter to only precache once per change
    final key = _lastFilterKey;
    if (key == null || _lastPrecacheKey == key) return;
    _lastPrecacheKey = key;
    // Precaches a handful of images near the top to avoid decode jank during initial scroll
    final count = products.length < 10 ? products.length : 10;
    for (var i = 0; i < count; i++) {
      final url = products[i].imageUrl;
      if (url.startsWith('data:image')) continue; // memory image, skip
      // Fire and forget; precache runs asynchronously
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        switch (_currentIndex) {
          case 1:
            pageContent = _buildReelsContent(context);
            break;
          case 2:
            ...
            SliverPadding(
              ...
              sliver: SliverLayoutBuilder(
                builder: (context, sliverConstraints) {
                  ...
                  // Opportunistically precache the first few images when filters change
                  _maybePrecacheTopGridImages(context, filteredProducts);
  ...
  Widget _buildReelsContent(BuildContext context) {
    return _ReelsView(
      reels: _combinedReels,
      savedReelIds: _savedReels,
      initialIndex: _reelStartIndex ?? 0,
      onInitialIndexConsumed: () {
        // Clear the start index once used
        _reelStartIndex = null;
      },
      onToggleSave: (id) => _toggleSaveReel(id),
  ...
  Widget _buildProfileContent(BuildContext context) {
    ...
    final favoriteReels = <_DemoReel>[];
    try {
      favoriteReels.addAll(_reels.where((r) => _savedReels.contains(r.id)));
    } catch (e) {
      // Handle error gracefully if _reels has issues
      // ignore for demo
    }
    ...
    return CustomScrollView(
      ...
      slivers: [
        ...
        ...switch (_profileSection) {
          ...
          _ProfileSection.saved => _buildSavedSlivers(context, favoriteReels),
          ...
          children: [
            _SettingsCard(children: [
              _settingsTile(
                ...
                onTap: () async {
                  ...
                  if (updated != null) {
                    ...
                    // Email may have changed; refresh seller status
                    _sellerEmailCtrl.text = updated.email;
                    await _refreshSellerStatus();
                  }
                },
              ),
              _settingsTile(
                ...
                subtitle: _checkingSellerForProfile
                    ? 'Checking...'
                    : (_sellerExistsForProfileEmail
                        ? 'Active for ${_profile.email}'
                        : 'Create your seller account'),
                onTap: () async {
                  if (_checkingSellerForProfile) return;
                  if (_sellerExistsForProfileEmail) {
                    // If seller already exists, switch immediately to dashboard.
                    await LocalSellerAccountsStore.setCurrentSeller(
                        _profile.email);
                    if (!mounted) return;
                    Navigator.push(
                      context,
  ...
  List<Widget> _buildSavedSlivers(
      BuildContext context, List<_DemoReel> favoriteReels) {
    final theme = Theme.of(context);
    if (favoriteReels.isEmpty) {
      return [
        SliverToBoxAdapter(
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 8),
                Text(
                  'Tap the bookmark or heart on a reel to save it here.',
                  style: theme.textTheme.bodyMedium?.copyWith(
                      color:
                          theme.colorScheme.onSurface.withValues(alpha: 0.7)),
                  textAlign: TextAlign.center,
    ...
    return [
      SliverToBoxAdapter(
        child: Padding(
          ...
          child: Row(
            children: [
              const Icon(Icons.movie, color: Colors.deepOrange),
              const SizedBox(width: 8),
              Text('Saved Reels',
                  style: Theme.of(context)
                      .textTheme
      ...
      SliverPadding(
        ...
        sliver: SliverGrid(
          ...
          delegate: SliverChildBuilderDelegate(
            (context, index) {
              final r = favoriteReels[index];
              return _SavedReelTile(
                reel: r,
                isSaved: true,
                onUnsave: () => _toggleSaveReel(r.id),
                onOpen: () {
                  // Open reels tab at this index
                  final all = _combinedReels;
                  final targetIndex = all.indexWhere((e) => e.id == r.id);
                  setState(() {
                    _reelStartIndex = targetIndex >= 0 ? targetIndex : 0;
                    _currentIndex = 1; // Reels tab (middle)
                  });
                },
              );
            },
            childCount: favoriteReels.length,
          ),
        ),
      ),
    ];
                ...
                child: ClipRRect(
                  ...
                  child: Stack(
                    ...
                    children: [
                      _AnyImage(
                        key: ValueKey(
                            'img-${product.id}-${product.updatedAt.millisecondsSinceEpoch}-${(product.images.isNotEmpty ? product.images.first : product.imageUrl)}'),
...
class _ReelPlayerCardState extends State<_ReelPlayerCard> {
  ...
  Future<void> _initController() async {
    final c = VideoPlayerController.networkUrl(Uri.parse(widget.reel.videoUrl));
    _controller = c;
    try {
      await c.initialize();
...
class _AnyImage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // If cache dimensions are provided, skip LayoutBuilder to reduce per-image layout work.
    if (cacheWidth != null && cacheHeight != null) {
      return networkOrPlaceholder(cw: cacheWidth, ch: cacheHeight);
    }

    // Fallback path: compute a reasonable cache size based on current constraints.
    return LayoutBuilder(
      builder: (context, constraints) {
        final dpr = MediaQuery.of(context).devicePixelRatio;
        final cw = (constraints.maxWidth.isFinite
